...
def main(config):
    ...
    """
    loss_test_avg, rmse_test_avg = test(config, logger, test_loader, model)
...
def test(config, logger, data_loader, model):
    ...
    # Listas para recopilar resultados en la CPU para ser más eficiente en memoria
    local_outputs_cpu = []
    local_targets_cpu = []
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            output = model(images)
            # Solo calcula la pérdida si la salida es finita
    ...
    if not dist.is_initialized() or dist.get_rank() == 0:
        ...
        if num_valid > 0:
            ...
            param_names = ["mDisk", "gamma", "psi"]
            log_str = " * Test RMSE por Parámetro (sobre datos válidos): "
